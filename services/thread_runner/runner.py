...
class ThreadRunner:
    ...
    async def run_thread(self, thread: Thread) -> Thread:
        ...
        self.slack.post_message("C08678XJ66C", f"🧵 *Thread Started* `{thread.thread_id[:8]}`\n**Goal**: {thread.goal}")
        ...
        try:
            ...
            while turn_count < max_turns:
                ...
                if tool_calls:
                    print(f"🛠️ Turn {turn_count}: Tool Calls detected: {[tc.name for tc in tool_calls]}")
                ...
                print(f"  💬 Text detected ({len(final_response_text)} chars). Prompting to proceed...")
            ...
            self.slack.post_message("C08678XJ66C", f"🚨 *Thread Failed* `{thread.thread_id[:8]}`\nError: {str(e)}")
