"""
DOAC Wisdom Ingestion Script for Dav1d / Unk Agent
...
# Paths - User to provide source
TRANSCRIPTS_DIR = Path("c:/Users/super/Watchtower/Dav1d/resources/transcripts") # Default fallback
...
def ingest_transcripts(
    ...
    print(f"\n{Colors.NEON_CYAN}🧠 DOAC Wisdom Ingestion for Dav1d{Colors.RESET}")
    print("=" * 60)
...
def search_wisdom(query: str, top_k: int = 5):
    ...
    for i, result in enumerate(results, 1):
        ...
        print(f"\n[{i}] Score: {score:.3f}")
        print(f"    Guest: {metadata.get('guest', 'Unknown')}")
        print(f"    {content}...")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DOAC Wisdom Ingestion for Dav1d")
    ...
    if args.search:
        ...
        transcripts_dir = Path(args.source) if args.source else TRANSCRIPTS_DIR
        ingest_transcripts(
        ...
        print("DOAC Wisdom Ingestion for Dav1d")
        print("=" * 40)
        print("Usage:")
        ...
        print("\nFirst, extract transcripts:")
        print("  python tools/youtube_transcript.py")
