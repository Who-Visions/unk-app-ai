...
class RobinhoodCryptoAPI:
    ...
    Usage:
        ...
        # HFT methods
        order_id = api.place_limit_buy("BTC-USD", 0.001, 50000.0)
    """
    ...
    def _request(
        ...
        # CRITICAL: Use compact separators for signature AND request body
        # Robinhood rejects if signature uses different whitespace than payload
        ...
        try:
            if method == "GET":
                ...
                # Pass data=body_str (bytes/str) instead of json=body
                # This ensures requests doesn't re-serialize with different whitespace
    ...
    def _request_paginated(self, method: str, path: str, body: Dict = None) -> List[Dict]:
        """Make authenticated API request and handle pagination."""
        all_results = []
        current_path = path
        
        while current_path:
            result = self._request(method, current_path, body)
            if not result:
                break
            
            # Some endpoints might not be paginated but still return results
            ...
            if next_url and self.BASE_URL in next_url:
                current_path = next_url.replace(self.BASE_URL, "")
            elif next_url and next_url.startswith("/api/"):
    ...
    def get_account(self) -> Dict[str, Any]:
        ...
        if self.api_version == 2:
            results = self._request_paginated("GET", path)
            if results:
                account = results[0]
    ...
    def get_holdings(self, *asset_codes: str) -> List[CryptoHolding]:
        ...
        if results:
            for h in results:
                # Handle keys that might vary (v1 vs v2)
                qty_val = h.get("quantity") or h.get("total_quantity") or 0
                avail_val = h.get("quantity_available") or h.get("quantity_available_for_trading") or 0
                
                # Fetch Cost Basis / Average Buy Price
                avg_price = h.get("average_buy_price") or h.get("cost_basis", {}).get("amount") # sometimes nested?
                # Actually for RH Crypto API: 'cost_basis' is often a struct or direct string?
                ...
                holdings.append(CryptoHolding(
                    ...
                    total_quantity=float(qty_val),
                    available_quantity=float(avail_val),
                    average_buy_price=float(avg_price) if avg_price else 0.0
                ))
    ...
    def get_order_as_dict(self, order_id: str) -> Dict[str, Any]:
        """Get order and return as dictionary for HFT compatibility."""
        ...
        return {
            ...
            "quantity": order.filled_quantity, # fallback
            "price": order.average_price, # fallback
            "symbol": order.symbol
        }
    
    def place_limit_buy(self, symbol: str, quantity: Any, limit_price: Any, client_order_id: str = None) -> str:
        """Convenience method for HFT buy orders."""
        order = self.place_limit_order(symbol, "buy", limit_price, asset_quantity=quantity, client_order_id=client_order_id)
        return order.order_id if order else ""
    
    def place_limit_sell(self, symbol: str, quantity: Any, limit_price: Any, client_order_id: str = None) -> str:
        """Convenience method for HFT sell orders."""
        order = self.place_limit_order(symbol, "sell", limit_price, asset_quantity=quantity, client_order_id=client_order_id)
        return order.order_id if order else ""
