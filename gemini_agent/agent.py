...
from skills import audio, synthesis, generation, web_tools
            ...
            if not response_config.thinking_config:
                if self.thinking_budget > 0:
                    ...
                    if self.thinking_level:
                        try:
                            setattr(response_config.thinking_config, 'thinking_level', self.thinking_level)
                        except AttributeError:
                            pass
            else:
...
class AgentFactory:
    ...
    async def create_routed(
        ...
        **kwargs
        ...
        from skills.notion_skill import NotionSkill  # Lazy import to avoid circular deps
        notion = NotionSkill()
