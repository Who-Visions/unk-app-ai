...
```mermaid
graph TD
    U[User Request] --> G[FastAPI Gateway]
    G --> A{OIDC Auth}
    A --> C[Intent Classifier]
    C --> Rtr[Model Router]

    subgraph CC[Cognitive Core]
        Rtr --> F[Gemini 2.0 Flash]
        Rtr --> T[Gemini 2.0 Flash Thinking]
        Rtr --> P[Gemini 2.5 Pro]
    end

    F --> X[Tool Execution Layer]
    T --> X
    P --> X

    subgraph SM[Skills and Memory]
        X --> N[Notion SDK]
        X --> S[Slack API]
        X --> V[Firestore Vector DB]
        X --> W[Web Search and Scrape]
    end
